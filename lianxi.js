define('search_new', ['jdf/1.0.0/unit/search/1.0.0/search', 'jdf/1.0.0/unit/getjsonp/1.0.0/getjsonp', 'product/module/clslog', 'jdf/1.0.0/ui/lazyload/1.0.0/lazyload', './search_plug.js?201710'], function(require, exports, module){
	...
	window.searchlog = getQueryString('forcebot') ? function(){} : function(){
		...
		var search_log_url = "//sstat.jd.com/scslog?args=", j = args.length, expand = '';
	...
	var searchGetShopId = function(shop_id, container) {
		var $shopDom = $('#J_shop_focus_' + shop_id, container);
	...
	//店铺加关注、登录弹窗
	searchUnit.shopFocus = function(shop_id, action, is_iframe) {
		//查找容器
		var container = is_iframe ? window.frames['shop_list'].document : document;
		//
		if ( action=='do' ) {
			//加关注
			checkLogin(function(){
				$.getJSON('//follow-soa.jd.com/rpc/vender/follow?sysName=search.jd.com&venderId='+shop_id+'&callback=?',function(res){
					if (typeof res=='object' && (res.data || res.code=='F0402')){
						var $shopDom = searchGetShopId(shop_id, container);
				...
				window.seajs.use(['jdf/1.0.0/ui/dialog/1.0.0/dialog'],function(dialog){
					$('body').dialog({
						...
						onSubmit: function(){
							...
							//取消关注
							$.getJSON('//follow-soa.jd.com/rpc/vender/unfollow?sysName=search.jd.com&venderId='+shop_id+'&callback=?',function(res){
			...
			//检查关注状态
			$.getJSON('//follow-soa.jd.com/rpc/vender/batchIsFollow?sysName=search.jd.com&venderIds='+shop_id+'&callback=?', function(res){
				if ( typeof res=='object' && typeof res.data=='object' && res.success ) {
	...
	SEARCH.get_shop_info = function() {
		...
		var shopid_list = [];
		//店铺id集合
		$('span.m-focus').not('[data-done="1"]').each(function(){ this.setAttribute('data-done', '1'); this.getAttribute('data-venderId') && shopid_list.push(this.getAttribute('data-venderId')); });
		//店铺信息填充
		shopid_list.length && $.getJSON('shop_new.php?ids=' + shopid_list.join(','), function(res){
			if ( typeof res=='object' && res.length ) {
				for (var i=0, j=res.length, target, tag, summary; i<j; i++) {
					...
					//加减关注
					target.find('.m-focus').click(function(){searchUnit.shopFocus($(this).attr('data-shopid'), $(this).hasClass('z-focused') ? 'undo' : 'do', 0)});
				}
			}
		})
		...
		//查询店铺关注状态
		shopid_list.length && searchUnit.shopFocus(shopid_list.join(','), 'check', 0);
	};
	...
	SEARCH.get_shop_name = function(tpl){//获取店铺名
		...
		for(var i=0, arr=[], pid; i<shop_length; i++) {
			...
			pids.push(pid);	el_dongdong[pid] = el_shops.eq(i);
			el_shops[i].setAttribute('data-done', '1');
		}
		if ( arr.length ) {
			$.getJSON('shop_new.php',{'ids':arr.unique().join(",")}, function(res){
				if( typeof res != 'object' ) { return; }
				for(var i=0,j=res.length,map={}; i<j; i++ ) { map[res[i].venderId] = res[i] }
				for(var i=0; i<shop_length; i++) {
					...
					//图书模板
					tpl=='2' && el_shop.removeAttr('data-shopid').find('a.curr-shop').replaceWith('<a class="curr-shop" target="_blank" onclick="searchlog(1,'+curr.shop_id+',0,58)" href="//mall.jd.com/index-'+curr.shop_id+'.html" title="'+curr.shop_name+'">'+curr.shop_name+'</a>');
					//3C、服装模板
					(tpl=='1' || tpl=='3') && el_shop.removeAttr('data-shopid').html('<span class="J_im_icon"><a target="_blank" onclick="searchlog(1,'+curr.shop_id+',0,58)" href="//mall.jd.com/index-'+curr.shop_id+'.html" title="'+curr.shop_name+'">'+curr.shop_name+'</a></span>');
				}
				//im图标
	...
	SEARCH.get_diviner_ware = function() { //少结果调推荐
		...
		//
		$.ajax({
			url: '//diviner.jd.com/diviner?p=907006&skus='+skus.join(',')+'&uuid=&pin='+(readCookie('pin')||'')
			...
			success: function(json) {
				...
				if(tpl=='1' || tpl=='2'){
					tpl_item += '<div class="p-operate">'+
						...
						'<a class="p-o-btn addcart" data-stock="{#sku#}" data-sv="1" data-disable-notice="0" data-presale="0" href="//gate.jd.com/InitCart.aspx?pid={#sku#}&pcount=1&ptype=1" target="_blank"><i></i>加入购物车</a></div>';
				}
				tpl_item += '</div></li>';
